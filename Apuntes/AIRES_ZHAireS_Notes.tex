...
\begin{equation}
	...
	\label{ec13}
\end{equation}
\subsection{Altura en la atmósfera, $h$, en función de distancia recorrida a lo largo del eje desde IP, $L$}
\begin{figure}[H]
	\centering
\begin{tikzpicture}[x=0.75pt,y=0.75pt,yscale=-1,xscale=1]
...
Debe usarse con cuidado la expresión anterior, que supone que el límite inferior de la integral es menor a $v$, i.e., \textit{integramos hacia arriba en la atmósfera}. Si tenemos cascadas hacia abajo, en que avanzar a lo largo del eje implica reducir la altura, aparecerá un signo extra. Peor aún, en cascadas estratosféricas la altura primero decrece y luego crece, así que según la región habrá un signo o no.
...
\begin{itemize}
	...
	\verb|upr_...| , \verb|RAS_...| (para RASPASS)
...
La norma general para las partículas comunes será usar \verb|prot|, \verb|elec|, \verb|gamma|. Para las demás partículas y núcleos, siempre el nombre completo: \verb|muon|, \verb|pion|, \verb|kaon|, \verb|lead|, ...
...
\end{itemize} 
\textbf{PELIGRO: Existe un límite de 64 caracteres para la longitud de los nombres para tasks, si se supera AIRES truncará la cadena de caracteres y perderemos información en los comentarios extra.}
...
\begin{itemize}
	...
	\begin{verbatim}
		upr_elec_1e18eV_01km_95deg_65degG_ZHSt_nconst
	\end{verbatim}
\item Cascada estratosférica iniciada por un fotón (RASPASS) de $1\,\mathrm{EeV}$, a ángulo cenital $93,25^\circ$ y azimut $0^\circ$. La trayectoria cruza la vertical del observador (en el Polo Sur) a $35\,\mathrm{km}$ de altura (RASPASSHeight) y el primario se inyecta a $1462.5\,\mathrm{km}$ de dicho cruce (RASPASSDistance). No se impone TimeShift, se calcula emisión en radio en ambos dominios con un campo magnético fijado.
\begin{verbatim}
	RAS_gamma_1e18eV_1462.5km_35km_00ns_93.25deg_00deg_SouthPole_ZHSb_fixB
\end{verbatim}

En este caso se pasa de 64 caracteres y se pierden comentarios extra. Nos encontraríamos esto en los outputs:
\begin{verbatim}
	RAS_gamma_1e18eV_1462.5km_35km_00ns_93.25deg_00deg_SouthPole_ZHS
\end{verbatim}
Hay que recortar el nombre eliminando los comentarios menos relevantes (\verb|_SouthPole|, por ejemplo, ya que el campo magnético que afecta a la simulación de emisión en radio está fijado previamente). 
\end{itemize}
